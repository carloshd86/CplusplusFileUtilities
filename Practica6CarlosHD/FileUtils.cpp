...
*/
unsigned int CountStringAppearances(const char * strToSearch, const FileIdentifier * fileId)
{
	assert(strToSearch);
	assert(fileId);
	...
	while (BUFFER_SIZE == (bytesRead = ReadFile(buffer, fileId, BUFFER_SIZE)))
	{
...
*/
int SumCommaSeparatedIntegers(const FileIdentifier * fileId)
{
	assert(fileId);
